...
See the [Basic Interface](#basic-interface) section below for how to perform common tasks with these models.

Please note that PuppetForge resources are identified by unique slugs rather than
...
------------- | ---------------------------------------------- | ----------------------------------------
modules       | Restricts the scope of queries to that of modules owned by this user  | <pre>>> user.modules.total<br>=> 78</pre>
...
provided by the API. For convenience, pagination links are exposed by the library.
...
An enumerator exists for iterating over the entire (unpaginated) collection. Keep in mind that this will result
